...
- **`linux/`** — Fedora KDE post-install notes, plus `networking/` drop-ins for DNS and transport tuning:
  ...
  - `nts/` and `unencrypted-nts/` are two **alternative** chrony configs — deploy one, not both. `nts/` is NTS-only (authenticated) and non-smearing; `unencrypted-nts/` is plain, unauthenticated NTP against Google and Amazon, which hide leap seconds behind a shared 24-hour linear smear (12:00 UTC → 12:00 UTC).
...
**Linux drop-ins** (pick one of `encrypted-dns/` or `proxy-dns/` for the DNS line, and one of `nts/` or `unencrypted-nts/` for the time line):
...
# or: sudo cp linux/networking/unencrypted-nts/chrony.conf /etc/chrony.conf
#     sudo cp linux/networking/unencrypted-nts/chronyd /etc/sysconfig/chronyd
...
- Linux side: systemd-resolved and NetworkManager (Fedora KDE defaults); `dnscrypt-proxy` only if using the `proxy-dns/` variant; chrony 4.6+ for the `nts/` variant, 4.0+ for `unencrypted-nts/`.
